...
=> 두 개의 select문의 조회되는 칼럼의 갯수가 같아야 한다. 
*/
...
/*
2. 함수
단일행함수 : 하나의 레코드에서만 처리되는 함수 - where 조건문에서 사용 가능함
그룹함수   : 여러 레코드에서 기능을 처리하는 함수 - having 조건문에서 사용 가능함
...
-- substr(컬럼명/문자열, 시작인덱스, [갯수]) : 시작 인덱스 부터 문자열 끝까지
-- left(컬러명/문자열, 갯수)       : 왼쪽부터 글자수만큼 부분 문자열로 리턴
-- right(컬러명/문자열, 갯수)      : 오른쪽부터 글자수만큼 부분 문자열로 리턴
-- 학생의 이름 2자만 조회하기
...
-- [문자추가함수] :lpad, rpad
-- lpad(칼럼, 전체자리수, 채울문자) : 컬럼을 전체자리수 출력시 빈자리는 왼쪽에 추가문자로 추가
-- rpad(칼럼, 전체자리수, 채울문자) : 컬럼을 전체자리수 출력시 빈자리는 오른쪽에 추가문자로 추가
-- 학생의 학번과 이름 출력
-- 학번: 10자리 빈자리 오른쪽: *
-- 이름: 10자리 빈자리 왼쪽:#
...
-- [문자 치환함수] : replace
-- replace(컬럼명|문자열, '문자1','문자2') : 컬럼의 값을 문자1를 문자 2로 치환
...
-- [그룹의 위치]
-- find_in_set : ,를 기준으로 문자열 그룹에서 문자가 몇번째 위치인지 리턴
...
*/
-- [반올림 함수 : round]
-- round(숫자,[소숫점이하자리수 생략가능]) : 소숫점이하 첫번째 자리에서 반올림하여 정수형으로 출력
...
-- [제곱 함수: power(숫자1, 숫자2)] : 숫자1을 숫자2만큼 곱하기 
SELECT 21/8, 21%8, MOD(21,8),POWER(3,3);

-- [날자 관련 함수]
-- 현재일시 : now()
-- 현재일자 : curdate(),curdate,courrent_date, courrent_date()
