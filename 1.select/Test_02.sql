...
SELECT REPLACE(NAME, SUBSTR(NAME,2,1), '#') AS '101번학과학생' FROM student WHERE major1 = 101;
...
SELECT name AS 이름,email AS 이메일,REPLACE(email, SUBSTR(email,5,3), '###') FROM professor;
-- 15. 교수테이블의  email 주소의 @앞의 3자리를 ###으로 치환하여 출력하기  교수의 이름, email, #mail을 출력하기
SELECT name AS 이름,email AS 이메일,REPLACE(email, SUBSTR(email,1,3), '###') FROM professor;
-- 16. 사원테이블에서 사원이름에 *를 왼쪽에 채운  6자리수 이름과, 업무와 급여를 출력한다.
SELECT LPAD(ename, 10,'*') AS 이름,job AS 업무,salary AS 급여 FROM emp; 
...
SELECT NAME AS 이름,RPAD(id,20,'$') AS id FROM professor 
UNION
SELECT NAME AS 이름,LPAD(id,20,'#') AS id FROM student
-- 21. 2025년 1월 10일 부터 2025년 5월 20일까지 개월수를 반올림해서 정수 출력하기
SELECT truncate(DATEDIFF('2025-05-20', '2025-01-10') / 30,0);
-- 22. EMP 테이블에서 10번 부서 직원의 현재까지의 이름, 입사일, 근무 월수를 계산하여   출력하기.  
--    근무월수 : 근무일수/30 반올림하여 정수로 출력하기
SELECT ename AS 이름,hiredate AS 입사일,TRUNCATE(DATEDIFF(NOW(), hiredate) / 30, 0) AS 근무월수 FROM emp WHERE ename = '김경선';